...
- Centralized all types and interfaces into `src/types/` with `.type.ts` convention
- Extracted utility functions into `src/utils/` with `.util.ts` convention
- Moved `MultipleInstanceManager` into `src/managers/` module
- Removed `RegistryCollection` adapter — `BaseRegistry` now uses `MapCollection` directly
...
- Added `ES2020` and `DOM` libs to tsconfig for `Map`, `URL`, `Buffer`, and `btoa` support
